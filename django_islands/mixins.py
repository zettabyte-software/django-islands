...
from .exceptions import ChangeTenantError, EmptyTenantError
...
class TenantModelMixin:
    ...
    def __setattr__(self, attrname, val):
        ...
        if (
            ...
            if not app_settings.ALLOW_TENANT_UPDATE:
                raise ChangeTenantError(
                    "Tenant column of a row cannot be updated. "
                    "You can set 'ALLOW_TENANT_UPDATE' to True in settings to allow this."
                )
    ...
    def _do_update(self, base_qs, using, pk_val, values, update_fields, forced_update):
        ...
        if current_tenant:
            ...
            if not app_settings.ALLOW_NULL_TENANT:
                raise EmptyTenantError(empty_tenant_message)
